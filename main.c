...
void readFileStoreInArray(FILE *fp, Customer *cu, int size)
{ // The function to read file data and store to customers struct array

    if (!fp)
    { // If there is an error occured while reading file
    ...
    while (fgets(line, 300, fp))
    {
        ...
        { //'\000'
            ...
            if (fieldCount == 0 && strcmp(cu[i].name, "") == 0)
            {                                               // If the name attribute is not assigned
            ...
            else if (fieldCount == 1 && strcmp(cu[i].surname, "") == 0)
            {                                              // If the surname attribute is not assigned
            ...
            else if (fieldCount == 2 && (strncmp(field, ",", 1) == 0))
            { // If the gender field is empty in the CSV file
                strcpy(cu[i].gender, "");
                field = field + 1;
            }

            else if (fieldCount == 2 && strcmp(cu[i].gender, "") == 0)
            {                                    // If the gender attribute is not assigned
            ...
            else if (fieldCount == 3 && (strncmp(field, ",", 1) == 0))
            { // If the occupancy field is empty in the CSV file

                strcpy(cu[i].occupancy, "");
                field = field + 1;
            }

            else if (fieldCount == 3 && strcmp(cu[i].occupancy, "") == 0)
            {                                              // If the occupancy attribute is not assigned
                char *comma_position = strchr(field, ','); // find the next location of the comma
                int length;
                if (comma_position != NULL)
                {                                    // The comma is found
            ...
            else if (fieldCount == 4 && (strncmp(field, ",", 1) == 0))
            { // If the level_of_education field is empty in the CSV file

                strcpy(cu[i].level_of_education, "");
                field = field + 1;
            }

            else if (fieldCount == 4 && strcmp(cu[i].level_of_education, "") == 0)
            {                                              // If the level_of_education attribute is not assigned
                char *comma_position = strchr(field, ','); // find the next location of the comma
                int length;
                if (comma_position != NULL)
                {                                    // The comma is found
            ...
            else if (fieldCount == 5 && (strncmp(field, ",", 1) == 0))
            { // If the email field is empty in the CSV file

                strcpy(cu[i].email, "");
                field = field + 1;
            }

            else if (fieldCount == 5 && strcmp(cu[i].email, "") == 0)
            {                                              // If the email attribute is not assigned
                char *comma_position = strchr(field, ','); // find the next location of the comma
                int length;
                if (comma_position != NULL)
                {                                    // The comma is found
            ...
            else if (fieldCount == 6 && (strncmp(field, ",", 1) == 0))
            { // If the bank_account_number field is empty in the CSV file

                strcpy(cu[i].bank_account_number, "");
                field = field + 1;
            }

            else if (fieldCount == 6 && strcmp(cu[i].bank_account_number, "") == 0)
            {                                              // If the bank_account_number attribute is not assigned
                char *comma_position = strchr(field, ','); // find the next location of the comma
                int length;
                if (comma_position != NULL)
                {                                    // The comma is found
            ...
            else if (fieldCount == 7 && (strncmp(field, ",", 1) == 0))
            { // If the IBAN field is empty in the CSV file

                strcpy(cu[i].IBAN, "");
                field = field + 1;
            }

            else if (fieldCount == 7 && strcmp(cu[i].IBAN, "") == 0)
            {                                              // If the IBAN attribute is not assigned
                char *comma_position = strchr(field, ','); // find the next location of the comma
                int length;
                if (comma_position != NULL)
                {                                    // The comma is found
            ...
            else if (fieldCount == 8 && (strncmp(field, ",", 1) == 0))
            { // If the account_type field is empty in the CSV file

                strcpy(cu[i].account_type, "");
                field = field + 1;
            }

            else if (fieldCount == 8 && strcmp(cu[i].account_type, "") == 0)
            {                                              // If the account_type attribute is not assigned
                char *comma_position = strchr(field, ','); // find the next location of the comma
                int length;
                if (comma_position != NULL)
                {                                    // The comma is found
            ...
            else if (fieldCount == 9 && (strncmp(field, ",", 1) == 0))
            { // If the currency_unit field is empty in the CSV file

                strcpy(cu[i].currency_unit, "");
                field = field + 1;
            }

            else if (fieldCount == 9 && strcmp(cu[i].currency_unit, "") == 0)
            {                                              // If the currency_unit attribute is not assigned
                char *comma_position = strchr(field, ','); // find the next location of the comma
                int length;
                if (comma_position != NULL)
                {                                    // The comma is found
            ...
            else if (fieldCount == 10 && (strncmp(field, ",", 1) == 0))
            { // If the total_balance_available field is empty in the CSV file

                strcpy(cu[i].total_balance_available, "");
                field = field + 1;
            }

            else if (fieldCount == 10 && strcmp(cu[i].total_balance_available, "") == 0)
            {                                              // If the total_balance_available attribute is not assigned
                char *comma_position = strchr(field, ','); // find the next location of the comma
                int length;
                if (comma_position != NULL)
                {                                    // The comma is found
            ...
            else if (fieldCount == 11 && (strncmp(field, ",", 1) == 0))
            { // If there is no emoji in the CSV file and the last character is ','

                strcpy(cu[i].available_for_loan, "");
                field = field + 1;
            }

            else if (fieldCount == 11 && strcmp(cu[i].available_for_loan, "") == 0)
            {                                               // If the available_for_loan attribute is not assigned
...
void printValues(Customer *c, int number)
{ // The function to print the elements of the customers struct array
...
bool writeToBinaryFile(FILE *fpb, Customer *data, int total)
{ // The function to create a binary file
...
Customer *readFromBinaryFile(char *fileName, int *tp)
{ // The function that returns the pointer of the array

    FILE *fpbr = fopen(fileName, "rb"); // Open the file

    if (fpbr == NULL)
        return NULL; // If there is a problem return NULL

    Customer cus;
    Customer *cp = &cus;

    while (fread(cp, sizeof(Customer), 1, fpbr) != 0)
    { // There is no problem while reading the struct
        *tp = *tp + 1;
    }
    ...
    if (fread(data, sizeof(Customer), *tp, fpbr) != *tp)
    {               // If there is a problem reading whole structs
        free(data); // Free the allocated space in memory
        return NULL;
    }

    if (fclose(fpbr) == EOF)
    {               // If there is a problem occured while closing the file
        free(data); // Free the allocated space in memory
        return NULL;
    }
...
unsigned int littleToBigEnd(int n)
{ // The function that converts the little endion version of the number to the big endion
    ...
    { // filling the array with 8 bit parts from end to beginning
        i = i - 8;
        for (int j = 0; j < 8; j++)
        {
...
void createXML(Customer *binaryData, int *tp)
{ // The function that creates Xml file
...
{ // XML validation with XSD
    ...
    char *XSDFileName = "control.xsd"; // write your xsd file here
...
int main(int argc, char *argv[])
{
    ...
    //////////////////////////////////////////////////////////////
    // CREATING XML FILE
    if (strcmp(argv[0], "./myConverter") == 0 && strcmp(argv[1], "records.dat") == 0 && strcmp(argv[2], "records.xml") == 0 && strcmp(argv[3], "2") == 0)
    { // myConverter records.dat records.xml 2
        ...
        if (binaryData == NULL)
        { // If there is a problem occured while reading binary file
    ...
    //////////////////////////////////////////////////////////////
    // XML VALIDATION WITH XSD
    if (strcmp(argv[0], "./myConverter") == 0 && strcmp(argv[1], "records.xml") == 0 && strcmp(argv[2], "records.xsd") == 0 && strcmp(argv[3], "3") == 0)
